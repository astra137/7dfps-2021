...
signal updated(move_dir, look_pitch, look_yaw)

var move_dir := Vector3.ZERO
var look_pitch := 0.0
var look_yaw := 0.0

mastersync func set_move(move: Vector3, pitch: float, yaw: float):
	move_dir = move
	look_pitch = pitch
	look_yaw = yaw
	emit_signal("updated", move_dir, look_pitch, look_yaw)
